...
static struct buf *rahead(struct inode *rip, block_t baseblock, u64_t position,
...
int fs_readwrite(void) {
	...
	if (block_spec && rw_flag == WRITING
			&& (dev_t) rip->i_zone[0] == superblock.s_dev
	...
	while (nrbytes > 0) {
		...
		r = rw_chunk(rip, cvul64((unsigned long) position), off, chunk, nrbytes,
...
int fs_breadwrite(void) {
	...
	while (nrbytes > 0) {
		...
		/* Read or write 'chunk' bytes. */
		r = rw_chunk(&rip, position, off, chunk, nrbytes, rw_flag, gid, cum_io,
 ...
 *				rw_chunk				     *
 *===========================================================================*/
static int rw_chunk(rip, position, off, chunk, left, rw_flag, gid, buf_off,
		block_size, completed)
	...
	*completed = 0;
	...
	if (!block_spec && b == NO_BLOCK) {
		if (rw_flag == READING) {
			...
			r = sys_safememset(VFS_PROC_NR, gid, (vir_bytes) buf_off, 0,
					(size_t) chunk);
			if (r != OK) {
...
# define BLOCKS_MINIMUM		(nr_bufs < 50 ? 18 : 32)
	...
	if (block_spec && rip->i_size == 0) {
		...
		blocks_left = (block_t)(
				rip->i_size - ex64lo(position) + (block_size - 1)) / block_size;
